...
def location(
    ...
    try:
        if str(f) == "None":
            ...
            session.commit()
            if epsg == 28992:
                strsql = "update regiodeal_timeseries.location set geom = st_setsrid(st_point(x,y),epsgcode) where geom is null"
            else:
                strsql = "update regiodeal_timeseries.location set geom = st_transform(st_setsrid(st_point(x,y),epsgcode),28992) where geom is null"
